...
role_lis = get_first_subdir(audio_roles_dir)
...
for file, pth in golden_res[:4]:
    ...
    corres_dir = os.path.join(audio_out_dir, f'{file_name}/voice')
    ...
    for audio_text,value in new_audio_dic.items():
        ...
        if audio_text in real_dic:
            ...
            new_aud_dir = os.path.join(audio_roles_dir, role)
...
# chaji_lis = [item for item in origin_lis if item not in same_lis]
# print(chaji_lis)
