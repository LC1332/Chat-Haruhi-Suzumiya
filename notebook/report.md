...
尽管本文更多讨论的是prompt设计和个性化语言生成的部分，但是一个良好的前端体验，很显然对于整体的项目有很大的提升作用。本项目意在建立多样化的前端，让更多不同操作系统环境的用户和开发者，可以体验Chat凉宫春日这个系统。我们最早的Chat凉宫春日，是使用gradio搭建的纯文字系统，在加入一定的图文对数据之后，可以实现在Haruhi酱输出语言之后。
建立凉宫春日应援会也有和我们合作桌面宠物的想法，将在后期执行。
